...
JBOSS_HOST_CONFIG=""
JBOSS_USER=""
JBOSS_DOMAIN_BASE_DIR=""
JBOSS_BIND=""
JBOSS_MANAGEMENT_BIND=""
JBOSS_OPTS=""
JBOSS_ENVIRONMENT=""
...
if [ -z "$JBOSS_ENVIRONMENT" ]; then
  JBOSS_ENVIRONMENT='/etc/profile.d/jboss.sh'
fi

if [ -r "$JBOSS_ENVIRONMENT" ]; then
  . ${JBOSS_ENVIRONMENT}
fi


if [ -z "$JBOSS_DOMAIN_BASE_DIR" ]; then
  JBOSS_DOMAIN_BASE_DIR='/opt/master/domain'
fi

if [ -z "$JBOSS_BIND" ]; then
  JBOSS_BIND="0.0.0.0"
fi

if [ -z "$JBOSS_MANAGEMENT_BIND" ]; then
  JBOSS_MANAGEMENT_BIND="0.0.0.0"
fi

if [ -z "$JBOSS_OPTS" ]; then
  JBOSS_OPTS="-Djboss.domain.base.dir=${JBOSS_DOMAIN_BASE_DIR} -b=${JBOSS_BIND} -bmanagement=${JBOSS_MANAGEMENT_BIND}"
fi
...
if [ "$JBOSS_MODE" = "standalone" ]; then
	...
	JBOSS_MARKERFILE=${JBOSS_DOMAIN_BASE_DIR}/tmp/startup-marker
fi
	...
	do
		...
		if [ -f $JBOSS_MARKERFILE ]; then
			...
			if [ "$markerfiletimestamp" -gt "$currenttime" ] ; then
				grep -i -e 'success:'  $JBOSS_MARKERFILE > /dev/null
				if [ $? -eq 0 ] ; then
					launched=true
					break
				fi
